...
> TODO: no database is installed yet. Fill in section 5 (Tech) of
> `blueprint/project-plan.md` with the chosen database/ORM, then update this
> section (Prisma conventions are a reasonable default if undecided).

## Data Fetching

- Server components fetch directly from the data layer
- Client components use Server Actions
- Validate all inputs with Zod

> TODO: no auth provider is installed yet. Once one is chosen, scope every
> user-owned query by the authenticated user id and document the exact field
> name here; never trust a client-supplied user id.
